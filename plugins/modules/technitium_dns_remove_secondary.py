...
seealso:
  - module: effectivelywild.technitium_dns.technitium_dns_delete_secondary
...
class RemoveSecondaryModule(TechnitiumModule):
    ...
    def run(self):
        ...
        if self_node and self_node.get('type') != 'Primary':
            self.fail_json(
                msg="This is not a Primary node. Secondary nodes can only be removed from the Primary node.",
