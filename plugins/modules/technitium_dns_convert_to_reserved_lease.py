...
DOCUMENTATION = r'''
---
module: technitium_dns_convert_to_reserved_lease
short_description: Convert a dynamic DHCP lease to a reserved lease
version_added: "0.6.0"
description:
...
seealso:
  ...
  - module: effectivelywild.technitium_dns.technitium_dns_add_reserved_lease
    description: Add a reserved DHCP lease
  - module: effectivelywild.technitium_dns.technitium_dns_remove_reserved_lease
    description: Remove a reserved DHCP lease
...
options:
    ...
    hardwareAddress:
        description:
            - The MAC address of the device bearing the dynamic lease
...
EXAMPLES = r'''
- name: Convert dynamic lease to reserved by MAC address
...
- name: Convert dynamic lease to reserved by client identifier
  technitium_dns_convert_to_reserved_lease:
    api_url: "http://localhost"
    api_token: "myapitoken"
    name: "Default"
...
msg:
    ...
    type: str
    returned: always
    sample: "Dynamic lease for MAC '00:11:22:33:44:55' converted to reserved in scope 'Default'."
'''

from ansible_collections.effectivelywild.technitium_dns.plugins.module_utils.technitium import TechnitiumModule


class ConvertToReservedLeaseModule(TechnitiumModule):
    argument_spec = dict(
    ...
    def run(self):
        ...
        # Convert the lease to reserved via the Technitium API
        data = self.request('/api/dhcp/leases/convertToReserved', params=query, method='POST')
        self.validate_api_response(data)

        # Return success - lease was converted
        self.exit_json(
            changed=True,
            msg=f"Dynamic lease for {identifier} converted to reserved in scope '{scope_name}'.",
            api_response=data
        )


if __name__ == '__main__':
    module = ConvertToReservedLeaseModule()
    module.run()
